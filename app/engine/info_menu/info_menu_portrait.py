...
class InfoMenuPortrait():
    ...
    def __init__(self, portrait, should_blink: bool = False, wink_instead: bool = False, blip: bool = False):
        ...
        self.timer = 7000 + random.choice(range(-2000, 2000, 125))

        self.should_blink = should_blink
        self.blink_counter = \
            counters.generic3counter(self.timer, utils.frames2ms(3), utils.frames2ms(15 if blip else 3))
        self.blink_counter.last_update = engine.get_time()

        self.wink_instead = wink_instead
        self.wink_counter = \
            counters.generic3counter(self.timer, utils.frames2ms(3), self.timer)
        self.wink_counter.last_update = engine.get_time()
    ...
    def create_image(self):
        ...
        if self.should_blink:
            if self.wink_instead:
                if self.wink_counter.count == 0:
                    wink_image = engine.subsurface(self.portrait.image, (PORTRAIT_WIDTH - 32, 64, 16, 16))
                    main_image.blit(wink_image, self.portrait.blinking_offset)
                elif self.wink_counter.count == 1:
                    wink_image = engine.subsurface(self.portrait.image, (PORTRAIT_WIDTH - 32, 48, 32, 16))
                    main_image.blit(wink_image, self.portrait.blinking_offset)
                elif self.wink_counter.count == 2:
                    wink_image = engine.subsurface(self.portrait.image, (PORTRAIT_WIDTH - 16, 64, 16, 16))
                    main_image.blit(wink_image, utils.tuple_add(self.portrait.blinking_offset, (16, 0)))
    ...
    def update(self):
        ...
        self.wink_counter.update(engine.get_time())
